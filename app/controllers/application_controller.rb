class ApplicationController < ActionController::API
     ...
     if token_decoded.length>1
       ...
       if renewed_payload['exp'].present?
         ...
         if time_now >= time_start.to_i && time_now <= time_end.to_i
           renewed_payload['exp'] = 12.hours.from_now.to_i
 ...
 private
   def jwt_decode(jwt_hash)
     begin
       JWT.decode jwt_hash, Rails.application.secrets.secret_jwt_auth, true, { :algorithm => 'HS256' }
   ...
   def jwt_encode(payload = {})
     JWT.encode payload, Rails.application.secrets.secret_jwt_auth, 'HS256'
   end
